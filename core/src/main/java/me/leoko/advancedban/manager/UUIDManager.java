...
public class UUIDManager {
    ...
    public String getNameFromUUID(String uuid, boolean forceInitial) {
        ...
        try (Scanner scanner = new Scanner(URI.create("https://api.mojang.com/user/profiles/" + uuid + "/names").toURL().openStream(), "UTF-8")) {
    ...
    private String askAPI(String url, String name, String key) throws IOException {
    	MethodInterface mi = mi();
        name = name.toLowerCase();
        HttpURLConnection request = (HttpURLConnection) URI.create(url.replaceAll("%NAME%", name).replaceAll("%TIMESTAMP%", new Date().getTime() + "")).toURL().openConnection();
        request.connect();
