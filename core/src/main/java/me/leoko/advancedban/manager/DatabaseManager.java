...
public class DatabaseManager {
    ...
    private synchronized ResultSet executeStatement(String sql, boolean result, Object... parameters) {
    	if (Universal.IS_DEBUG_BUILD) {
    		StringBuilder dbg = new StringBuilder("SQL: ").append(sql);
    		if (parameters.length > 0) {
    			dbg.append(" | params=[");
    			for (int i = 0; i < parameters.length; i++) {
    				if (i > 0) dbg.append(", ");
    				dbg.append(parameters[i]);
    			}
    			dbg.append(']');
    		}
    		Universal.get().debug(dbg.toString());
    	}
    	try (Connection connection = dataSource.getConnection(); PreparedStatement statement = connection.prepareStatement(sql)) {
    		...
    		Universal.get().log(
   					"An unexpected error has occurred executing an Statement in the database\n"
    						...
    						+ "error in your fork's issue tracker (see plugin description)"
    				);
