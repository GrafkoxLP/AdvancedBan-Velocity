...
public enum Command {
    ...
    ADVANCED_BAN(null,
            ...
            input -> {
                ...
                mi.sendMessage(sender, "§8§l§m-=====§r §c§lAdvancedBan §8§l§m=====-§r ");
                mi.sendMessage(sender, "  §cOriginal §8• §7Leoko");
                mi.sendMessage(sender, "  §cMaintainer §8• §7Leon (Grafkox_LP)");
                mi.sendMessage(sender, "  §cStatus §8• §a§oStable");
                mi.sendMessage(sender, "  §cVersion §8• §7" + mi.getVersion());
                mi.sendMessage(sender, "  §cLicense §8• §7GPL-3.0");
                mi.sendMessage(sender, "  §cStorage §8• §7" + (DatabaseManager.get().isUseMySQL() ? "MySQL (external)" : "HSQLDB (local)"));
                mi.sendMessage(sender, "  §cServer §8• §7" + (Universal.get().isBungee() ? "Proxy (Bungee/Velocity)" : "Spigot/Bukkit"));
                mi.sendMessage(sender, "  §cBuild §8• §7" + Universal.BUILD_TAG);
